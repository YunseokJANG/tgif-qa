...
from models.frameqa_base import *
from ops import *
...
class FrameQATp(FrameQABase):
    ...
    def build_graph(self,
        ...
        with tf.variable_scope("loss") as scope:
            embed_state = tf.contrib.layers.fully_connected(final_embed, self.answer_size, activation_fn=None)
...
class FrameQASp(FrameQABase):
    ...
    def build_graph(self,
        ...
        self.acc_min = tf.reduce_mean(tf.stack(self.acc_min_list, axis=0))
        self.acc_max = tf.reduce_mean(tf.stack(self.acc_max_list, axis=0))
    ...
    def build_graph_single_gpu(self, video, video_mask, caption, caption_mask, answer, idx):
        ...
        with tf.variable_scope("att_image_emb"):
            ...
            batch_pre_att = tf.reshape(video, [self.batch_size_per_gpu*self.lstm_steps, 7*7, self.channel_size])
        ...
        with tf.variable_scope("video_rnn") as scope:
            video_cell = rnn.MultiRNNCell([self.get_rnn_cell() for i in range(self.num_layers)])
            vid_state = video_cell.zero_state(self.batch_size_per_gpu, tf.float32)

            for i in range(self.lstm_steps):
                if i > 0:
                    scope.reuse_variables()
                new_output, vid_state = video_cell(embedded_feat_drop[:, i, :], vid_state)
        ...
        with tf.variable_scope("loss") as scope:
            rnn_final_state = tf.concat([cap_state[1][0], cap_state[1][1]], 1)
            embed_state = tf.contrib.layers.fully_connected(rnn_final_state, self.answer_size, activation_fn=None)
        ...
        with tf.variable_scope("accuracy"):
            ...
            acc_min = tf.reduce_mean(tf.cast(correct_predictions, "float"), name="accuracy_min_%d"%idx)
            acc_max = tf.reduce_mean(tf.cast(correct_predictions, "float"), name="accuracy_max_%d"%idx)
