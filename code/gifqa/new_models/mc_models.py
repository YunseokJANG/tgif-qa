...
class MCTp(MCBase):
    ...
    def build_graph(self,
        ...
        with tf.variable_scope("video_rnn") as scope:
            ...
            vid_state = self.video_cell.zero_state(self.agg_batch_size, tf.float32)
            vid_states = []
...
class MCSp(MCBase):
    ...
    def build_graph_single_gpu(self, video, video_mask, question, question_mask, answer, idx):
        ...
        with tf.variable_scope("caption_rnn") as scope:
            ...
            cap_state = caption_cell.zero_state(self.agg_batch_size, tf.float32)
        ...
        with tf.variable_scope("att_image_emb"):
            video_2d = tf.reshape(video_agg, [self.agg_batch_size*self.lstm_steps*7*7 ,self.channel_size])
            ...
            #  [batch_size, length, channel_size]
            embedded_feat_drop = tf.nn.dropout(embedded_feat, self.dropout_keep_prob_t)

        with tf.variable_scope("video_rnn") as scope:
            video_cell = rnn.MultiRNNCell([self.get_rnn_cell() for i in range(self.num_layers)])
            vid_state = video_cell.zero_state(self.agg_batch_size, tf.float32)
